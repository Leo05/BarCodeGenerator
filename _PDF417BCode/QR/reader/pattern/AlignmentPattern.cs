using System;
using QRCodeDecoder = _PDF417BCode.QRCode.Codec.QRCodeDecoder;
using _PDF417BCode.QRCode.Codec.Reader;
using AlignmentPatternNotFoundException = _PDF417BCode.QRCode.ExceptionHandler.AlignmentPatternNotFoundException;
using InvalidVersionException = _PDF417BCode.QRCode.ExceptionHandler.InvalidVersionException;
using _PDF417BCode.QRCode.Geom;
using _PDF417BCode.QRCode.Codec.Util;

namespace _PDF417BCode.QRCode.Codec.Reader.Pattern
{
	...
	public class AlignmentPattern
	{
		...
		internal static Point[][] getCenter(bool[][] image, FinderPattern finderPattern, Point[][] logicalCenters)
		{
            ...
            canvas.drawCross(centers[0][0], _PDF417BCode.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.UR);
			centers[sqrtCenters - 1][0] = axis.translate(- 3, 3);
            canvas.drawCross(centers[sqrtCenters - 1][0], _PDF417BCode.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.DL);
			centers[0][sqrtCenters - 1] = axis.translate(3, - 3);
            canvas.drawCross(centers[0][sqrtCenters - 1], _PDF417BCode.QRCode.Codec.Util.Color_Fields.BLUE);
			...
			for (int y = 0; y < sqrtCenters; y++)
			{
				for (int x = 0; x < sqrtCenters; x++)
				{
					...
					if (y == 0)
					{
                        ...
                        canvas.drawCross(centers[x][y], _PDF417BCode.QRCode.Codec.Util.Color_Fields.RED);
					}
					else if (x == 0)
					{
                    ...
                    canvas.drawCross(centers[x][y], _PDF417BCode.QRCode.Codec.Util.Color_Fields.BLUE);
                    canvas.drawLine(new Line(tmpPoint, centers[x][y]), _PDF417BCode.QRCode.Codec.Util.Color_Fields.LIGHTBLUE);
